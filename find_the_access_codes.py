...
#number of passcodes changes daily. Commander Lambda gets a report every day that includes the locks' 
#access codes, but only she knows how to figure out which of several lists contains the access codes. 
...
#With that information, you can figure out which list contains the number of access codes 
#that matches the number of locks on the door when you're ready to go in (for example, 
#if there's 5 passcodes, you'd need to find a list with 5 "lucky triple" access codes).

#Write a function answer(l) that takes a list of positive integers l and counts the 
#number of "lucky triples" of (lst[i], lst[j], lst[k]) where i < j < k.  The length of l is between 2 and 2000 inclusive.  
